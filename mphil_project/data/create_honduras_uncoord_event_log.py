...
def create_honduras_coord_event_log(data):
    ...
    length_df = earliest_events_df.groupby('case:concept:name').filter(lambda x: len(x) >= 2 and len(x) <= 5)
    ...
    num_variants_to_keep = int(0.01 * len(variants))
...
if __name__ == "__main__":
    ...
    pm4py.write_xes(filtered_log, Path(__file__).parent.parent.parent / "data/processed/honduras_uncoordinated_log.xes")
